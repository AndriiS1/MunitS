...
public class GetPartUploadUrlQueryHandler(IOptions<StorageOptions> options,
    ...
    IObjectByUploadIdRepository objectByUploadIdRepository) : IRequestHandler<GetPartUploadUrQuery, GetPartUploadUrlResponse>
{
    ...
    public async Task<GetPartUploadUrlResponse> Handle(GetPartUploadUrQuery query, CancellationToken cancellationToken)
    {
        ...
        var @object = await objectByUploadIdRepository.GetByUploadId(bucket.Id, Guid.Parse(query.Request.ObjectId), Guid.Parse(query.Request.UploadId));
        ...
        var dataToSign = SignatureRule.GetSignature(bucket.Id.ToString(), @object.Id.ToString(), query.Request.UploadId, query.Request.PartNumber, expirationUnix);
