    ...
    IPathRetriever pathRetriever) : IRequestHandler<CreateObjectCommand, ObjectServiceStatusResponse>
{
    public async Task<ObjectServiceStatusResponse> Handle(CreateObjectCommand command, CancellationToken cancellationToken)
    {
        ...
        var objectVersions = await objectByFileKeyRepository.GetAll(command.Request.FileKey, bucket.Id);
        ...
        if (objectVersions.Count > 0)
        {
            if (bucket.VersioningEnabled)
            {
                ...
                {
                    await objectByFileKeyRepository.Delete(command.Request.FileKey, bucket.Id, objectVersions.Last().VersionId);
        ...
        await objectByFileKeyRepository.Create(newObject);
